...
const { withFonts } = require('../index.js');
...
it('should configure a unique rule correctly', () => {
    const webpackConfig = withFonts()().webpack(createConfig(), options());
...
it('should configure a unique rule correctly (prod)', () => {
    const webpackConfig = withFonts()().webpack(createConfig(), options({ dev: false }));
...
it('should call nextConfig webpack if defined', () => {
    ...
    const webpackConfig = withFonts()(nextConfig).webpack(createConfig(), options());
